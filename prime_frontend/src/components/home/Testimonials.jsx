const Testimonials = ({ data }) => {
  if (!data || data.length === 0) {
    return null;
  }

  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <span key={i} className={`text-sm ${i < rating ? 'text-[#F4C430]' : 'text-gray-300'}`}>
        ★
      </span>
    ));
  };

  return (
    <section className="bg-[#F6F4EE] py-16 px-6"> {/* Changed to your background color */}
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold text-center text-[#111111] mb-4">
          Client Testimonials
        </h2>
        <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
          Hear what our satisfied clients have to say about our printing services
        </p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.map((testimonial, index) => (
            <div 
              key={index} 
              className="relative bg-white p-8 rounded-xl shadow-sm group overflow-hidden transition-all duration-500 hover:-translate-y-2"
            >
              {/* Water fill animation background */}
              <div className="absolute inset-0 bg-linear-to-t from-[#1E88C8]/0 via-[#1E88C8]/0 to-[#1E88C8]/0 group-hover:from-[#1E88C8]/5 group-hover:via-[#1E88C8]/10 group-hover:to-[#1E88C8]/15 transition-all duration-500 ease-out translate-y-full group-hover:translate-y-0"></div>
              
              {/* Main content container */}
              <div className="relative z-10">
                {/* Avatar on top - centered */}
                <div className="flex justify-center mb-6">
                  <div className="relative group">
                    {/* Avatar border animation */}
                    <div className="absolute -inset-2 rounded-full bg-linear-to-r from-[#1E88C8] to-[#1E88C8]/50 opacity-0 group-hover:opacity-100 blur-sm transition-opacity duration-500"></div>
                    
                    <img
                      src={testimonial.avatar_url || '/default-avatar.png'}
                      alt={testimonial.client_name}
                      className="relative w-20 h-20 rounded-full object-cover border-4 border-white shadow-lg group-hover:border-[#1E88C8] transition-colors duration-300"
                      onError={(e) => {
                        e.target.src = '/default-avatar.png';
                      }}
                    />
                    
                    {/* Verified badge */}
                    <div className="absolute -bottom-2 -right-2 bg-[#F4C430] text-white w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-md">
                      ✓
                    </div>
                  </div>
                </div>

                {/* Name and role - centered below avatar */}
                <div className="text-center mb-4">
                  <h4 className="font-bold text-lg text-[#111111] mb-1 group-hover:text-[#1E88C8] transition-colors duration-300">
                    {testimonial.client_name}
                  </h4>
                  <p className="text-sm text-gray-600 group-hover:text-gray-800 transition-colors duration-300">
                    {testimonial.client_role && testimonial.company
                      ? `${testimonial.client_role}, ${testimonial.company}`
                      : testimonial.company || testimonial.client_role || ''}
                  </p>
                </div>

                {/* Rating - centered with hover effect */}
                <div className="flex justify-center mb-6">
                  <div className="flex items-center space-x-1 px-4 py-2 bg-[#F6F4EE] rounded-full group-hover:bg-[#1E88C8]/10 transition-colors duration-300">
                    {renderStars(testimonial.rating)}
                  </div>
                </div>

                {/* Quote with animated quotation marks */}
                <div className="relative">
                  <div className="absolute -top-4 -left-3 text-4xl text-[#1E88C8]/20 group-hover:text-[#1E88C8]/40 transition-colors duration-500">"</div>
                  
                  <div className="relative">
                    <p className="text-gray-700 leading-relaxed text-center italic relative z-10 group-hover:text-[#111111] transition-colors duration-300 min-h-30 flex items-center justify-center">
                      {testimonial.quote}
                    </p>
                    
                    {/* Bottom accent line that grows on hover */}
                    <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-linear-to-r from-transparent via-[#1E88C8] to-transparent group-hover:w-3/4 transition-all duration-500"></div>
                  </div>
                  
                  <div className="absolute -bottom-4 -right-3 text-4xl text-[#1E88C8]/20 group-hover:text-[#1E88C8]/40 transition-colors duration-500">"</div>
                </div>

                {/* Bottom accent with magenta on hover */}
                <div className="mt-8 pt-6 border-t border-gray-100 group-hover:border-[#E91E78]/30 transition-colors duration-300">
                  <div className="text-xs text-gray-400 group-hover:text-[#1E88C8] transition-colors duration-300 text-center font-medium">
                    Verified Client • Prime Print Customer
                  </div>
                </div>
              </div>
              
              {/* Corner accent on hover */}
              <div className="absolute top-0 right-0 w-12 h-12 overflow-hidden">
                <div className="absolute top-0 right-0 w-8 h-8 bg-[#E91E78] transform translate-x-4 -translate-y-4 rotate-45 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Testimonials;